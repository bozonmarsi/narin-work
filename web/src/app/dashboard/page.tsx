...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      {loadError && <p className="text-red-600 dark:text-red-400">Ошибка загрузки: {loadError}</p>}

      {attention.length > 0 && (
        <section className="space-y-2 rounded-lg border border-red-200 bg-red-50 dark:bg-red-500/10 p-4">
          <h2 className="font-semibold text-red-700 dark:text-red-400">Требует внимания ({attention.length})</h2>
          <div className="space-y-1">
            {attention.map((o) => (
              <div key={o.id} className="flex items-center gap-2">
                <button
                  onClick={() => setSelectedOrder(o)}
                  className="flex-1 rounded-md bg-white dark:bg-zinc-900 px-3 py-2 text-left text-sm hover:border-accent"
                >
                ...
                <button
                  ...
                  className="rounded-md border border-red-200 px-2 py-2 text-red-500 dark:text-red-400 hover:bg-red-100"
                >
                  ✕
                </button>
              </div>
