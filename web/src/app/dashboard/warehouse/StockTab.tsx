...
import type { BatchRow, RawMaterial } from "./types";
...
export function StockTab() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [batchRes, matRes] = await Promise.all([
        supabase
          .from("batches")
          .select("id, product_sticker_id, remaining, purchase_date, estimated_wilt_date, photo_url")
          .gt("remaining", 0)
          .order("estimated_wilt_date", { ascending: true, nullsFirst: false }),
        supabase.from("product_stickers").select("id, product_name, material_type, unit, default_vase_life_days").eq("category", "ohapka"),
      ]);
  ...
  return (
    <div className="max-w-2xl space-y-2">
      {batches.map((b) => {
        const m = materialById.get(b.product_sticker_id);
        ...
        return (
          <div key={b.id} className="flex items-center justify-between gap-3 rounded-md border border-zinc-200 dark:border-zinc-700 p-3 text-sm">
            <div>
              <p className="font-medium">{m ? decodeHtmlEntities(m.product_name) : "—"}</p>
              <p className="text-xs text-zinc-400">
                {b.remaining} {m?.unit ?? ""} · с {new Date(b.purchase_date).toLocaleDateString("ru-RU")}
              </p>
            </div>
